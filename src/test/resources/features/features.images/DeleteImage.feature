@Campyuk @Alifa
  Feature: Testing DELETE image as host
    ...
    Scenario: API DELETE IMAGE as HOST with VALID path and parameter should succeed then return 204 No Content response code
      ...
      Given Base URL is _ and set "1" as jsonPath camp
      When Set method to GET, Set path to detail camp, and click send button
      And Set image id to a method with "1" as jsonPath in delete an image
    ...
    Scenario: API DELETE IMAGE as HOST with valid path but INVALID PARAMETER should succeed then return 401 Unauthorized response code
      Given Base URL is _ and input json body log in "testingmhost2" as username and "testh234" as password
      When Set method to POST, Set path to login, and click send button
      And Set token to a method in delete an image
    ...
    Scenario: API DELETE IMAGE as HOST with valid path but WITH STRING PARAMETER should succeed then return 404 Not Found response code
      Given Base URL is _ and input json body log in "testingmhost2" as username and "testh234" as password
      When Set method to POST, Set path to login, and click send button
      And Set token to a method in delete an image
    ...
    Scenario: API DELETE IMAGE as HOST with valid path but WITHOUT PARAMETER should succeed then return 405 Method Not Allowed response code
      Given Base URL is _ and input json body log in "testingmhost2" as username and "testh234" as password
      When Set method to POST, Set path to login, and click send button
      And Set token to a method in delete an image
    ...
    Scenario: API DELETE IMAGE as HOST with valid path but with INVALID TOKEN should succeed then return 401 Unauthorized response code
