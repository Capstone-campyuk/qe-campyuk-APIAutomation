@Campyuk
  Feature: Testing POST add new camp as host
    ...
    Scenario Outline: API POST as HOST ADD NEW CAMP with EMPTY LATITUDE, LONGITUDE, DISTANCE should return response code 201 Created
      Given Base URL is _ and input json body log in host
      When Set method to POST, Set path to login, and click send button
      Given Base URL, set token to bearer token, and input form-data body price null, city "Bogor", latitude null, longitude null, address "keren", distance null, title "bagus", description "wow", images, and document
      When Set method to POST, Set path to camps, and click send button
      Then API should return response 201
      Then API should return body message "success add new camp"
      Examples:
        | price | city    | latitude | longitude    | address                                                                                    | distance | title                  | description                                                                   |
        | 20000 | "Bogor" | null     | 106.82477778 | "Jalan Bumi Cimandala Permai, Cimandala, Kec. Sukaraja, Kabupaten Bogor, Jawa Barat 16710" | 10       | "Cimandala Scout Camp" | "Tempat Kemah Pramuka. Lapangan luas dan aman. Tersedia aula dan tempat MCK." |